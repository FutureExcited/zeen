...
namespace {
    ...
    auto PlaceCard(size_t index, const BeTexture& texture) -> CardCorridorLobby::MemoryCard {
        ...
        return CardCorridorLobby::MemoryCard{
    ...
    public:
        MemorySleevePass(BeStandardRenderMachine* srm,
            ...
            const std::vector<CardCorridorLobby::MemoryCard>* cards)
    ...
    private:
        ...
        const std::vector<CardCorridorLobby::MemoryCard>* _cards;
    };
...
CardCorridorLobby::CardCorridorLobby(App* app, std::string name, std::string cardsDir)
    : Lobby(app, std::move(name)), _cardsDir(std::move(cardsDir)) {
    // Default spawn at the near end of the corridor, looking down it. Subclasses
    // may override spawn + portals in their own constructors.
    _defaultSpawnPos = {0.0f, 1.75f, -6.0f};
    _defaultSpawnLook = {0.0f, 1.95f, 0.0f};
}

CardCorridorLobby::~CardCorridorLobby() = default;

auto CardCorridorLobby::Prepare() -> void {
    if (_prepared) return;
    _prepared = true;
    ...
    _uniformMaterial->SetFloat3("WorldColor", WorldColor());
    ...
    const auto cardsDir = root / _cardsDir;
    ...
    _machine->DeclareTexture(Tex("Bloom"), SenFormat::R11G11B10_Float, 1.0f, false, 5);
    _machine->DeclareTexture(Tex("FXAA"), SenFormat::R11G11B10_Float);

    _machine->UniformMaterial = _uniformMaterial;
}

auto CardCorridorLobby::AddCardFromImage(const std::filesystem::path& imagePath) -> void {
    try {
...
auto CardCorridorLobby::OnEnter() -> void {
    ...
    _machine->AddPass(std::make_unique<MemorySleevePass>(
        _machine.get(), _machine->GetRenderTexture(Tex("HDR")), _machine->GetRenderTexture(Tex("Depth")), &_cards));
    _machine->AddBloomPass(5, Tex("HDR"), Tex("Bloom"), BeAssetRegistry::GetTexture("black").lock());

    auto fxaaMaterial = BeMaterial::Create("fxaa-material", false);
    fxaaMaterial->SetTexture("ColorTexture", _machine->GetRenderTexture(Tex("Bloom")));
    _machine->AddFullscreenPass(BeAssetRegistry::GetShader("fxaa"), fxaaMaterial, {Tex("FXAA")});
    _machine->AddBackbufferPass(Tex("FXAA"), BackbufferClear());
    _machine->BuildPasses();

    // Drag-drop a new card while in this lobby. The card uses the shared, already-
    // baked card mesh, so NO rebake is needed (re-baking leaks the old GPU buffers
    // and eventually exhausts VRAM — vmaCreateBuffer fails).
    auto* self = this;
...
auto CardCorridorLobby::Tick(float deltaTime) -> void {
...
auto CardCorridorLobby::SubmitFrame(float now) -> void {
